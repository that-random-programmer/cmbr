...
use crate::{parser::Parser, tokenizer::Tokenizer, treewalker::{self, RuntimeError, RuntimeErrorType, Treewalker}, types::{DiagnosticPrinter, Info, InterpreterIO, Span}};

#[wasm_bindgen]
pub fn run(source: &str, read_line: Function, print_hook: Function) {
    let io = WasmIO { read_line_hook: read_line, print_hook};
    let diag_printer = DiagnosticPrinter::new(source, io.clone());
    let tknr = Tokenizer::new(source.to_string(), "<editor>");
    let mut tkns = Vec::new();
    for tkn in tknr {
        match tkn {
            Ok(v) => if let Some(t) = v {
                tkns.push(t)
            },
            Err(e) => {
...
impl InterpreterIO for WasmIO {
    ...
    fn print(&self, s: &str) {
        self.print_hook.call1(&JsValue::NULL, &JsValue::from_str(&s.replace("\n", "\r\n"))).unwrap();
    }
    ...
    fn read_line(&self, span: Span) -> Result<String, RuntimeError> {
        match self.read_line_hook.call0(&JsValue::NULL) {
            ...
            Err(e) => Err(RuntimeError { msg: format!("internal error: {e:?}"), span, ty: RuntimeErrorType::InternalError, info: vec![Info::note("please report this error")] }),
        }
    }
